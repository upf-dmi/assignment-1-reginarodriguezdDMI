...
pacman::p_load(tidyverse, readxl, janitor, httr, knitr, flextable)
...
clean_data <- clean_data %>%
  mutate(
    ...
    # Set 0 and negative values to NA (to match original paper's range starting from 1.0)
...
# 2. Combine (Smoke & Alcohol data not available in dataset)
...
# Create flextable
flextable(kable_table) %>%
  set_caption("Table 1 Replication (Logic adjusted to Paper)") %>%
  autofit() %>%
  theme_booktabs() %>%
...
cat("\nDISCREPANCY NOTE:\n")
cat("This replication does not match Table 1 exactly due to differences between the\n")
cat("public supplementary file and the authors' dataset (e.g., COVID-19: 39M/26F here\n")  
cat("vs. 41M/24F published). Smoking/Alcohol data are also missing from the public file.\n")
cat("These discrepancies are common when replicating from public supplementary materials.\n")

# Save cleaned data for subsequent exercises (1.2, 2.1, 2.2, 3)
saveRDS(clean_data, "clean_data_ex1.rds")
